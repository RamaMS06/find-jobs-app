...
///
/// * use one [RShimmer] to wrap list of [Widget]s instead of a list of many [RShimmer]s
///
@immutable
class RShimmer extends StatefulWidget {
  ...
  /// [RShimmer] which [gradient] is [LinearGradient] made up of `baseColor` and
  /// `highlightColor`.
  ///
  RShimmer.fromColors({
    super.key,
  ...
  @override
  _RShimmerState createState() => _RShimmerState();
...
class _RShimmerState extends State<RShimmer> with SingleTickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(RShimmer oldWidget) {
    if (widget.enabled) {
      _controller.forward();
    } else {
      _controller.stop();
