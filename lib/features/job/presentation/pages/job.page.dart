...
class _HomePageState extends ConsumerState<HomePage>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (timeStamp) {
        ref.read(jobControllerProvider.notifier).findJobs();
        if (_scrollController.hasClients) {
          _scrollController.addListener(
            () {
              if (_scrollController.offset > 0) {
                ref.read(homeShowSearchBarShadow.notifier).state = true;
              }
              if (_scrollController.offset < 10) {
                ref.read(homeShowSearchBarShadow.notifier).state = false;
              }
            },
          );
        }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: RefreshIndicator(
        ...
        strokeWidth: 3,
        onRefresh: () async {
          await ref.read(jobControllerProvider.notifier).findJobs(
        ...
        child: CustomScrollView(
          ...
          slivers: [
            ...
            SliverAppBar(
              elevation: ref.watch(homeShowSearchBarShadow) ? 6 : 0,
              shadowColor: RColor.background.dark.withOpacity(0.2),
              pinned: true,
              toolbarHeight: 110,
              expandedHeight: 110,
              backgroundColor: RColor.background.white,
